...
def WR(ticker, startdate, enddate):
    df = pro.daily(ts_code=ticker,
                   start_date=startdate,
                   end_date=enddate)
    df_index = pd.to_datetime(df["trade_date"])
    df_index = df.sort_index()
    df["WR"] = 100 * (df["high"].rolling(14).max() - df["close"]) / (
            df["high"].rolling(14).max() - df["low"].rolling(14).min())
    return df


def WR_Strategy(ticker, startdate, enddate, buy_threshold=80, sell_threshold=20, cost=0):
    df = WR(ticker, startdate, enddate)
    df.loc[(df.WR > buy_threshold) & (
            df["open"] > df["close"].shift(1) * 0.903), "signal"] = 1  # 当威廉指数值大于80且第二天开盘没有跌停发出买入信号设置为1
    df.loc[(df.WR < sell_threshold) & (
            df["open"] < df["close"].shift(1) * 1.097), "signal"] = 0  # 当威廉指数值小于20且第二天开盘没有涨停发出卖入信号设置为0
    df.dropna(inplace=True)  # 去除无法计算的点
...
zgyh_WR = WR_Strategy("601988.SH", "20170101", "20211231")
plt.figure(figsize=(12, 6))
plt.plot(zgyh_WR['capital_line'])
